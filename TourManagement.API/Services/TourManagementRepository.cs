...
namespace TourManagement.API.Services
{
    public class TourManagementRepository : ITourManagementRepository
    {
        ...
        public async Task<IEnumerable<Tour>> GetToursForManager(int managerId, bool includeShows = false)
        {
            if (includeShows)
            {
        ...
        public async Task<Tour> GetTour(int tourId, bool includeShows = false)
        {
            if (includeShows)
            {
        ...
        public async Task<bool> IsTourManager(int tourId, int managerId)
        {
...
#pragma warning restore 1998
        ...
        public async Task<IEnumerable<Show>> GetShows(int tourId)
        {
            return await _context.Shows.Where(s => s.TourId == tourId).ToListAsync();
        }

        public async Task<IEnumerable<Show>> GetShows(int tourId, IEnumerable<int> showIds)
        {
        ...
        public async Task AddShow(int tourId, Show show)
        {
